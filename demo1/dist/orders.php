...
if(isset($_GET['add']) and !empty($_POST))
{
	...
	$siparisNo			    = generateUniqueOrderNumber($con);
	$urunNo 				= $_POST['urun-no'];
	$musteri 				= $_POST['musteri'];
	$adet 					= $_POST['adet'];
	$siparisTarih			= date("Y/m/d");
	...
	if($urun_sorgu->execute()) {
		$sorgu_result = $urun_sorgu	->get_result();
		if($sorgu_result->num_rows > 0) {
			$urun = $sorgu_result->fetch_object();
		} else {
		header('Location: ?msg=1');
		exit;
		}
		$urun_fiyat = 	isset($urun->urun_fiyat) ? $urun->urun_fiyat : 0; 
	}
	if(!empty($adet) and !empty($urun->urun_fiyat))
	{
		$adet = (int)$_POST['adet'];
$urun_fiyat = (float)$urun->urun_fiyat;
$ucret = $adet * $urun_fiyat;
	}
	else {
		$ucret = 0;
	}
			...
			$stmt = $con->prepare("INSERT INTO siparisler (siparis_no,urun_no,musteri,adet,siparis_tarih,teslimat_tarih,ucret,durum) values(?,?,?,?,?,?,?,?)");
			$stmt->bind_param('sssissii',$siparisNo,$urunNo,$musteri,$adet,$siparisTarih,$teslimatTarih,$ucret,$durum);
			if($stmt->execute())
			{
				$logId = $_SESSION['user_id'];
				$logData = "Product No: ".$urunNo." Product Name: ".$urunAd." Product Quantity : ".$urunAdet;
				$action = "Added";
							...
							<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
								...
								<div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
									...
									<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
										<!--begin::Title-->
										<h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Sipariş Listesi</h1>
										<!--end::Title-->
										<!--begin::Breadcrumb-->
										...
										<div class="card-header border-0 pt-6">
											...
											<div class="card-title">
												...
												<div class="d-flex align-items-center position-relative my-1">
													...
													<input type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-solid w-250px ps-12" placeholder="Ürün Ara" 
...
if(isset($_GET['id'],$_GET['product'])  )
{
	...
	$decrypted_data = coz($data);
	if ($decrypted_data === false) {
	
	} else {
		echo "value='".$decrypted_data."'";
	}
	
}
?>												/>

													<a href="?" class="btn btn-success p-3 d-flex justify-content-center aling-items-center" style="margin-left: 2%"><i class="fa-duotone fa-filter-slash p-0" style="font-size:16px;color:#fff !important;"></i></a>
												</div>
												<!--end::Search-->
											</div>
											<!--begin::Card title-->
																...
																<form id="kt_modal_add_user_form" class="form" action="?add=1" enctype="multipart/form-data" method="POST">
																	...
																	<div class="d-flex flex-column scroll-y px-5 px-lg-10" id="kt_modal_add_user_scroll" data-kt-scroll="true" data-kt-scroll-activate="true" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_user_header" data-kt-scroll-wrappers="#kt_modal_add_user_scroll" data-kt-scroll-offset="300px">
																		...
																		<div class="fv-row mb-7">
																			<!--begin::Label-->
																			<label class="required fw-semibold fs-6 mb-2">Ürün No</label>
																			<!--end::Label-->
																			<!--begin::Input-->
																			<input type="text" name="urun-no" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Ürün No"  />
																			<!--end::Input-->
																		</div>
																		<!--end::Input group-->
																		<!--begin::Input group-->
																		<div class="fv-row mb-7">
																			<!--begin::Label-->
																			<label class="required fw-semibold fs-6 mb-2">Müşteri</label>
																			<!--end::Label-->
																			<!--begin::Input-->
																			<input type="text" name="musteri" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Müşteri"  />
																			<!--end::Input-->
																		</div>
																		<div class="fv-row mb-7">
																			<!--begin::Label-->
																			<label class="required fw-semibold fs-6 mb-2">Adet</label>
																			<!--end::Label-->
																			<!--begin::Input-->
																			<input type="number" name="adet" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Adet"  />
																			<!--end::Input-->
																		</div>
																		<!--end::Input group-->
																		<div class="fv-row mb-7">
																			<!--begin::Label-->
																			<label class="required fw-semibold fs-6 mb-2">Teslimat Tarihi</label>
																			<!--end::Label-->
																			<!--begin::Input-->
																			<input type="date" name="teslimat_tarih" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Teslimat Tarihi"  />
																			<!--end::Input-->
																		</div>
																	</div>
																	<!--end::Scroll-->
										...
										<div class="card-body py-4">
											...
											<thead>
													...
													<th class="text-center min-w-100px">Sipariş No</th>
														<th class="min-w-200px">Ürün</th>
														<th class="text-center min-w-100px">Ürün No</th>
														<th class="text-center min-w-70px">Adet</th>
														<th class="text-center min-w-100px">Fiyat</th>
														<th class="text-center min-w-100px">Müşteri</th>
														<th class="text-center min-w-100px">Sipariş Tarih</th>
														<th class="text-center min-w-100px">Teslimat Tarih</th>
														<th class="text-center min-w-100px">Durum</th>
												...
												if(isset($_GET['id']))
												{
													...
													$stmt = $con->prepare("SELECT * FROM siparisler where id = ? ORDER BY create_time DESC");
													$stmt->bind_param("i",$id);
													
												}
												else {
														$stmt = $con->prepare("SELECT * FROM siparisler ORDER BY create_time DESC");
											...
											?>
													...
													<a href="order-edit.php?id=<?=$encrypted_id?>" class="text-gray-800 text-hover-primary fs-5 fw-bold" data-kt-ecommerce-product-filter="product_name">	#<?=$row->siparis_no?></a>
														</td>
														<td>
															<div class="d-flex align-items-center">
																<!--begin::Thumbnail-->
																<a href="assets/product-image/<?=$urun->image?>" class="symbol symbol-50px">
																	<span class="symbol-label" style="background-image:url(assets/product-image/<?=$urun->image?>);"></span>
																</a>
																<!--end::Thumbnail-->
																<div class="ms-5">
																	<!--begin::Title-->
																	<span class="text-gray-800 text-hover-primary fs-5 fw-bold" ><?=$urun->urun_ad?></span>
														...
														<td class="text-center pe-0" data-order="0">
															
															<?=$row->adet?>
														</td>
														<td class="text-center pe-0"><?=number_format($row->ucret,1,',','.')?> ₺</td>
														<td class="text-center pe-0"><?=$row->musteri?> </td>
														<td class="text-center pe-0"><?=$row->siparis_tarih?> </td>
														<td class="text-center pe-0"><?=$row->teslimat_tarih?> </td>
														<td class="text-center pe-0"><?php if($row->durum > 0){echo'<span class="badge badge-light-success">Tamamlandı</span>';}else{echo'<span class="badge badge-light-info">Üretimde</span>';}?> </td>
														...
														<td class="text-center">
															...
															<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
																<!--begin::Menu item-->
																<div class="menu-item px-3">
																	<a href="order-edit.php?id=<?=$encrypted_id?>" class="menu-link px-3">Düzenle</a>
																</div>
																<!--end::Menu item-->
																<!--begin::Menu item-->
																<div class="menu-item px-3">
																	<a href="#" class="menu-link px-3" data-kt-ecommerce-product-filter="delete_row" data-id="<?=$encrypted_id?>">Sil</a>
																</div>
																<!--end::Menu item-->
															</div>
															<!--end::Menu-->
...
$('.delete-data').click(function(){
    ...
    var confirmalert = confirm("Silmek İstediğine Emin Misin?");
    if (confirmalert == true) {
      $.ajax({
        ...
        success: function(response){
          ...
          if(response == 1){
            $(el).closest('tr').css('background','#FF8080');
	...
	return {
		init: function() {
            ...
            $('#kt_ecommerce_products_table thead th').each(function(index) {
                var title = $(this).text();
                if (index !== 1 && index !==3  && index !==4  && index !== 8 && index !== 9  ) { // 1 ve 4 sütunlarına arama eklenmez
            ...
            e.columns().every(function(index) {
                ...
                if (index !== 2 && index !==3 && index !== 4 && index !== 8) { // 1 ve 4 sütunlarına arama eklenmez
                    var that = this;
